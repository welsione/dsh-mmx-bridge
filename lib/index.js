...
export default {
  ...
  apply(ctx) {
    ...
    // 一键安装：npm install -g mmx-cli（跟随系统 npm 配置，不改 registry）
    function runMmxInstall() {
      ...
      let child
      try {
        child = spawnCmd('npm', ['install', '-g', 'mmx-cli'], { stdio: ['ignore', 'pipe', 'pipe'] })
      } catch (e) {
    ...
    function runMmxAuthStatus() {
      return new Promise((resolve) => {
        ...
        child.on('close', (code) => {
          ...
          if (code === 0) {
            // mmx auth status 输出 JSON：整理成易读的一行（key 由 mmx 自行打码）
            let nice = '已登录'
            try {
              const j = JSON.parse(text)
              if (j !== null && typeof j === 'object') {
                const kv = typeof j.key === 'string' && j.key ? '，key ' + j.key.slice(0, 14) : ''
                if (j.method === 'api-key') nice = '已登录（api-key' + (typeof j.source === 'string' && j.source ? '，来源 ' + j.source : '') + kv + '）'
