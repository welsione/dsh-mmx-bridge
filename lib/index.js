...
export default {
  ...
  apply(ctx) {
    ...
    const opts = {
      ...
      //   POST <setEnabledPath> → { plugin: vision|websearch|readimage, enabled }
                ...
                disposers.push(ws.register({
                  ...
                  handler: (req, res) => {
                    ...
                    req.on('end', () => {
                      try {
                        const parsed = JSON.parse(body || '{}')
                        const plugin = parsed.plugin === 'websearch' ? 'websearch' : parsed.plugin === 'readimage' ? 'readimage' : 'vision'
                        const enabled = typeof parsed.enabled === 'boolean' ? parsed.enabled : true
                        // 合并写入：保留 count / webSearchEnabled / readImageEnabled 等字段
                        const prev = readControl()
