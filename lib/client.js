...
// mmx_bridge 自定义工具视图：图片缩略图 + 音频/视频内嵌播放器（在当前页面直接播放）
		...
		function MmxSettingsCard() {
			...
			const card = react.createElement("li", { className: "dpm_card" },
				react.createElement("div", { className: "dpm_header" },
				...
				react.createElement("div", { className: "dpm_body" },
		...
		function apply(ctx) {
			...
			ctx.inject(["slots"], (scope) => {
				...
				scope.slots.inject("settings.plugin.item", () =>
					scope.slots.register(
						{ name: "settings.plugin.item", id: "dsh-mmx-bridge", order: 30 },
